...
PlayerFramework.Player = PlayerFramework.Object.extend({init: function (a, b) {this._super(); this.mergeOptions(b, {containerClassName: "pf-container",autoplay: false,initialVolume: 0.5, overlayControls: true, overlayPlayButton: true, plugins: PlayerFramework.Plugins, pluginOptions: {}}); this.plugins = []; this.modules = []; this.fallbackMediaPlugins = []; this.mediaPlugin = null; this.controls = []; this.isReady = false; PlayerFramework.domReady(PlayerFramework.proxy(this, function () {if (!a) throw Error("Element is null.");if (typeof a === "string" && (a = document.getElementById(a), !a))throw Error("Element not found.");if (a.tagName != null)a.className === this.options.containerClassName ? (this.containerElement = a, this.containerElement.style.width = this.options.width, this.containerElement.style.height = this.options.height) : a.parentNode.className !== this.options.containerClassName && this.createParentContainer(a); this.loadPlugins();this.options.mediaPluginFallbackOrder || this.setMediaPluginFallbackOrderFromDeclaration(a);(!this.options.mediaPluginFallbackOrder ||
